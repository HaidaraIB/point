...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body = await req.json().catch(() => ({})) as QicardBody;
    ...
    if (action === "get-settings") {
      ...
      const status = await getQicardSettingsStatus(
        saAccessToken,
    ...
    if (action === "save-settings") {
      await assertOsAdmin(saAccessToken, caller.firebaseProjectId, caller.uid);
      try {
        const status = await saveQicardSettings(
          {
    ...
    console.error("qicard error:", msg);
