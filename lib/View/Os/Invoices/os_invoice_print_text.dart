...
String buildOsInvoicePrintHtml(OsInvoiceModel invoice) {
  final ref = OsFinanceFormat.invoiceRef(invoice);
  final agency = _esc(AppLocaleKeys.osInvoicesAgencyHeader.tr);
  final rows = <String>[];
  if (invoice.items.isEmpty) {
    final amt = invoice.amount > 0 ? invoice.amount : invoice.total;
    rows.add(
      '<tr><td>1</td><td>${_esc(AppLocaleKeys.osInvoicesItemsFallback.tr)}</td>'
      '<td style="text-align:center">1</td>'
      '<td>${_esc(OsFinanceFormat.money(amt))}</td>'
      '<td>${_esc(OsFinanceFormat.money(amt))}</td></tr>',
    );
  } else {
    for (var i = 0; i < invoice.items.length; i++) {
      final it = invoice.items[i];
      rows.add(
        '<tr><td>${i + 1}</td><td>${_esc(it.description)}</td>'
        '<td style="text-align:center">${it.quantity}</td>'
        '<td>${_esc(OsFinanceFormat.money(it.unitPrice))}</td>'
        '<td>${_esc(OsFinanceFormat.money(it.total))}</td></tr>',
      );
    }
  }
  ...
  if (Get.isRegistered<OsStampSettingsController>()) {
    ...
    if (stamp.stampEnabled.value) {
      ...
      stampBlock =
          ...
          '${_esc(stamp.stampText.value)}</div>';
    }
  }

  final vatRow = invoice.vat > 0
      ? '<div>${_esc(AppLocaleKeys.osInvoicesVat.tr)}: '
          '${_esc(OsFinanceFormat.money(invoice.vat))}</div>'
      : '';
...
<head>
<meta charset="utf-8"/>
<title>$agency — ${_esc(ref)}</title>
<style>
  body { font-family: Tahoma, Arial, sans-serif; color: #0f172a; padding: 24px; }
  ...
  .stamp { display: inline-block; margin-top: 20px; border: 2px solid; padding: 10px 18px;
           border-radius: 12px; font-weight: 800; transform: rotate(2deg); }
  @media print { body { padding: 0; } }
</style>
</head>
<body>
  <h1>$agency</h1>
  <div>${_esc(AppLocaleKeys.osInvoicesNumber.tr)}: <strong>${_esc(ref)}</strong></div>
  <div class="meta">
    <div>${_esc(AppLocaleKeys.osInvoicesClient.tr)}: <strong>${_esc(invoice.clientName)}</strong></div>
    <div>${_esc(AppLocaleKeys.osInvoicesStatus.tr)}: ${_esc(OsFinanceFormat.invoiceStatusLabel(invoice.status))}</div>
    <div>${_esc(AppLocaleKeys.osInvoicesDate.tr)}: ${_esc(invoice.date)}</div>
    <div>${_esc(AppLocaleKeys.osInvoicesDueDate.tr)}: ${_esc(invoice.dueDate)}</div>
  </div>
  <table>
    <thead>
      <tr>
        <th>#</th>
        <th>${_esc(AppLocaleKeys.osInvoicesItemDesc.tr)}</th>
        <th>${_esc(AppLocaleKeys.osInvoicesQty.tr)}</th>
        <th>${_esc(AppLocaleKeys.osInvoicesUnitPrice.tr)}</th>
        <th>${_esc(AppLocaleKeys.osInvoicesLineTotal.tr)}</th>
      </tr>
    </thead>
    <tbody>
      ${rows.join('\n')}
  ...
  <div class="totals">
    <div>${_esc(AppLocaleKeys.osInvoicesAmount.tr)}: ${_esc(OsFinanceFormat.money(invoice.amount))}</div>
    $vatRow
    <div class="grand">${_esc(AppLocaleKeys.osInvoicesTotal.tr)}: ${_esc(OsFinanceFormat.money(invoice.total))}</div>
  </div>
  $stampBlock
</body>
</html>
